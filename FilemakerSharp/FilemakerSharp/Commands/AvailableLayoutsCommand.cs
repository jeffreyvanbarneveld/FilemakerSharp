...
{
    class AvailableLayoutsCommand
    {
        private Filemaker m_fm;

        internal AvailableLayoutsCommand(Filemaker fm)
        {
            m_fm = fm;
        }
        ...
        /// Execute command
        /// </summary>
        /// <returns>Available layouts</returns>
        public async Task<List<string>> Execute()
        {
            List<string> databases = new List<string>();

            XmlNode xmlResponse = await m_fm.Communicate("-layoutnames", true);
